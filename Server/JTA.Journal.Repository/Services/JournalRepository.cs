...
namespace JTA.Journal.DataContext
{
    public class JournalRepository : IJournalRepository
    {
        ...
        public Task<(IEnumerable<TradeComposite>, Pagination)> GetAllTradeCompositesAsync(int pageNumber = 1, int pageSize = 10)
        {
            return _tradeCompositeRepository.GetAllTradeCompositesAsync(pageNumber, pageSize);
        }

        public Task<(IEnumerable<TradeComposite>, Pagination)> GetFilteredTradesAsync(TradesFilterModel filter, int pageNumber = 1, int pageSize = 10)
        {
